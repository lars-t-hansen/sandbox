...
--
--  For a spec of the output, see any of the Huffman encoders written in other
--  languages below the parent directory.
--
--  This program has only the encoding part, and currently only single-threaded.
...
procedure Huff_Ada is
   ...
   --  (bit string width; encoded size).  When raised, the encoder should just
   ...
   --  length.
   --
   --  TODO: It's possible that the array should be indirect.
   ...
   procedure Compute_Frequencies(Input : Buffer; Freqs : out Freq_Table) is
      ...
      procedure Sort_Freq is new Ada.Containers.Generic_Constrained_Array_Sort (Freq_Array_Range, Freq_Item, Freq_Array);
   ...
   begin
      return (Left.Weight < Right.Weight) or else (Left.Weight = Right.Weight and then Left.Serial < Right.Serial);
   end ">";
   ...
   begin
      ...
      for i in 0 .. Freqs.Length - 1 loop
         ...
         begin
            HuffHeap.Insert (Priq, Huff_Item'(Freqs.It (ix).Count, Serial, new Huff_Node'(Freqs.It (ix).Ch, null, null)));
            Serial := Serial + 1;
         end;
      end loop;
      while HuffHeap.Length (Priq) > 1 loop
         ...
         begin
            ...
            HuffHeap.Insert (Priq, Huff_Item'(a.Weight + b.Weight, Serial, new Huff_Node'(0, a.Tree, b.Tree)));
            Serial := Serial + 1;
         end;
      end loop;
      declare
   ...
   procedure Put1(A : in out MetaBuffer; V : Unsigned_64) is
   begin
      A.It (A.Length) := Unsigned_8 (V);
      A.Length := A.Length + 1;
   end Put1;

   procedure Put2(A : in out MetaBuffer; V : Unsigned_64) is
   begin
   ...
   procedure Put4(A : in out MetaBuffer; V : Unsigned_64) is
   begin
      A.It (A.Length) := Unsigned_8 (V and 255);
      A.It (A.Length + 1) := Unsigned_8 (Shift_Right(V, 8) and 255);
   ...
   -- Block compressor.
   --
   -- When Compress_Block returns normally, the Output and the Metadata together
   -- hold the compressed representation.
   --
   -- Compress_Block does not raise any exceptions.
   ...
   begin
      ...
      begin
         ...
         Put2 (Meta, Unsigned_64 (Freqs.Length));
         for i in 0 .. Freqs.Length-1 loop
            Put1 (Meta, Unsigned_64 (Freqs.It (Freq_Array_Range (i)).Ch));
            Put4 (Meta, Unsigned_64 (Freqs.It (Freq_Array_Range (i)).Count));
         end loop;
         Put4 (Meta, Unsigned_64 (Input.Length));
      exception
         when Cant_Encode =>
      ...
      Put4 (Meta, Unsigned_64 (Output.Length));
   end Compress_Block;
   ...
   begin
      ...
      while not FIO.End_Of_File(Input_File) loop
         --  This is completely tragic.  Surely there has got to be a better way than
         --  byte-at-a-time?
...
begin
   --  TODO: Grab file names from the command line.
   declare
      Input_Name  : String := "test.txt";
      Output_Name : String := "test.txt.huff";
   begin
      Compress_File (Input_Name, Output_Name);
   end;
end Huff_Ada;
